...
import { calculateHours, formatHours } from '@/lib/domain';
...
const emptyForm: FormState = { name: '', start_time: '16:00', end_time: '23:00', position: '1' };
    ...
    <div>
      ...
      <section className="card">
        ...
        <div className="actions" style={{ marginTop: 0 }}>
          <button className="secondary-button" onClick={() => setForm({ name: '1º turno', start_time: '16:00', end_time: '23:00', position: '1' })}>Modelo 2 turnos: 1º</button>
          <button className="secondary-button" onClick={() => setForm({ name: '2º turno', start_time: '23:00', end_time: '06:00', position: '2' })}>Modelo 2 turnos: 2º</button>
          <button className="ghost-button" onClick={() => setForm({ name: '1º turno', start_time: '06:00', end_time: '14:00', position: '1' })}>Modelo 3 turnos</button>
        </div>
        <div className="table-wrap">
          <table>
            <thead>
              <tr><th>Posição</th><th>Nome</th><th>Início</th><th>Fim</th><th>Horas</th><th>Ações</th></tr>
            </thead>
            <tbody>
              {shifts.map((shift) => (
                <tr key={shift.id}>
              ...
              {shifts.length === 0 ? <tr><td colSpan={6} className="muted">Nenhum turno cadastrado.</td></tr> : null}
            </tbody>
          </table>
        </div>
      </section>
